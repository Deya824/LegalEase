...
export default function LawyerTransactionsPage() {
  ...
  const fetchTransactions = async () => {
    ...
    fetch(`${process.env.NEXT_PUBLIC_SERVER_URL}/api/transactions/lawyer/${session.user.id}`, {
      headers: {
  ...
  const totalEarned = transactions.reduce((sum, t) => sum + (Number(t.fee) || 0), 0);
  ...
  return (
    <div>
      ...
      <div className="bg-white/5 border border-white/10 rounded-2xl p-6 mb-6 inline-block">
        <p className="text-gray-400 text-sm font-bold mb-1">Total Earned</p>
        <p className="text-3xl font-extrabold text-green-400">${totalEarned}</p>
      </div>
            ...
            {transactions.length === 0 ? (
              ...
              transactions.map((tx) => (
                <tr key={tx._id} className="border-b border-white/5">
                  ...
                  <td className="p-4">{tx.clientName || tx.clientEmail}</td>
